...
- `load_profile`: unconditional `self.profile_force_behavior = forced if forced in AMBIENT_DISPATCH_BEHAVIORS else None`, warning on the else. **Amended 2026-07-18 (Task 2 quality review):** validation is against `music_light.AMBIENT_DISPATCH_BEHAVIORS` (the 12 per-name-dispatchable ambient renderers), NOT `dmx_engine.VALID_BEHAVIORS` — the 7 punchy names all collapse into `_render_loopback_direct`, which ignores the behavior name, so pinning them would make the IPC `behavior` label report a renderer that isn't running. The unconditional shape also clears a stale pin if a pin-less profile is ever loaded into the same process.
- `_dispatch`: **Amended 2026-07-18:** `_detect_auto_behavior` RUNS every frame (keeping the energy state machine and Intent mood fresh); the forced behavior overrides only the returned *choice*. The original "skip it entirely" wording contradicted the spec's own invariant below. Invariant (unchanged): **pinned behavior never changes, palette variety still evolves.**
...
2. **Kick pulse.** On `is_kick`: the pulse level arms to `velocity_brightness(self._beat_velocity)/255` and master snaps instantly to the composited brightness. **Amended 2026-07-18 (Task 1 quality review):** compositing is additive over the floor — `brightness = min(1.0, floor + pulse·dimmer·(1−floor))` — because the original `max(floor, pulse·dimmer)` swallowed soft/mid kicks entirely under a loud floor (measured delta 0.0 at dimmer 50). The pulse then decays exponentially (`PULSE_KICK_DECAY` 0.10 per frame ≈ ~500ms perceptual) from its own level — no hold plateau, no step-up (double-pulse lesson). Re-entry (discontinuity) zeroes the pulse and snare timer so a stale hit never replays.
